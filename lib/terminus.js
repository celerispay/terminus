...
function decorateWithHealthCheck (server, state, options) {
  const { healthChecks, logger, onSendFailureDuringShutdown, caseInsensitive, authentication } = options
  let basicAuthHash = ''
  if (authentication && authentication.username && authentication.password) {
    basicAuthHash += Buffer.from(authentication.username + ':' + authentication.password, 'utf-8').toString('base64')
  }
  let hasSetHandler = false
  const createHandler = (listener) => {
    const check = hasSetHandler ? () => {} : async (healthCheck, req, res) => {
      if (state.isShuttingDown) {
        return sendFailure(res, { onSendFailureDuringShutdown })
      }
      const { headers } = req
      const authHeader = headers.authorization ? headers.authorization.split(' ') : ''
      if (authHeader && authHeader[0] !== 'Basic') {
        const errorAuthType = ['Invalid auth mechanism']
        return sendFailure(res, { errorAuthType })
      }
      if (authHeader && authHeader[1] !== basicAuthHash) {
        const errorAuth = ['Invalid username/password']
        return sendFailure(res, { errorAuth })
      }
      let info
      try {
    ...
    return async (req, res) => {
      ...
      if (healthCheck) {
        return check(healthCheck, req, res)
      } else {
        listener(req, res)
      }
    }
